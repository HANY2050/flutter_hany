...
class _AllProductsState extends State<AllProducts>
    with AutomaticKeepAliveClientMixin<AllProducts> {
  var page = 1;
  ...
  void initState() {
    super.initState();
    ProductsApi().fetchProducts(page).then((dataFormServer) {
      setState(() {
        data = dataFormServer;
      });
    });
  ...
  void _onRefresh() async {
    ...
    var List = await ProductsApi().fetchProducts(page);
    //Clear Old data
    data.clear();
    // page;
    // Add New data
  ...
  void _onLoading() async {
    print(_onLoading);
    page++;
    var List = await ProductsApi().fetchProducts2(page);
    data.addAll(List);
    await Future.delayed(Duration(seconds: 1));
    setState(() {
